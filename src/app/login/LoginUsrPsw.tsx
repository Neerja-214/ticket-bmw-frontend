...
import utilities, { svLnxSrvr, createGetApi, dataStr_ToArray, setCmpId, setUsrId } from "../../utilities/utilities";
...
const LoginUsrPsw = () => {
  ...
  const reqFlds = [{ fld: "eml", msg: "Enter User Name", chck: "length" },
  { fld: "otp", msg: "Enter OTP", chck: "length" }];
  ...
  const CheckOtp = () => {
    ...
    //console.log(msg)
    if (msg && msg[0]) {
      dispatch({ type: ACTIONS.CHECK_REQ, payload: msg[0] });
      return;
    }
  ...
  const GetData = () => {
    ...
    let usr : string = GetDataValue(state.textDts, "eml")
    api = createGetApi("dll=x|fnct=a130|db=nodb",   usr + '=' + api);
    ...
    if (data && data.data && data.data[0] && data.data[0]["Data"]) {
      ...
      if (ary && ary[0]['id']){
        if(Number(ary[0]['id'])>100){
          ...
          setUsrId( ary[0]['usrid'])
          sessionStorage.setItem("lvl", ary[0]['lvl'])
          sessionStorage.setItem("lvlofc", ary[0]['lvlofc'])
          sessionStorage.setItem("mainid", ary[0]['id'])
          let nm : number = Number(ary[0]['lvl'])
          if (nm == 100){
            setTimeout(function(){
              navigate("/cntr")
              // navigate("/dshnw")
          ...
          } else if (nm == 1) { //Regional Directorate
            setTimeout(function(){
              sessionStorage.setItem("rgndirct", ary[0]['usrid'])
              navigate("/rgnd")
            },800)
          } else if (nm == 200) { //Regional Directorate
          ...
          dispatch({type: ACTIONS.CHECK_REQ, payload : "Invalid Login Credentials !!!!"})
          setTimeout(function(){
  ...
  return (
    ...
    <div className="log-in1" style={{ marginTop: "0px", width:"25%"}}>
        ...
        {/* <div className="flex" style={{ marginTop: "10px", justifyContent: "center", alignItems:"center" }}>
          <Button
            size="medium"
            style={{ backgroundColor: "#0F8EC6", textAlign: "center", width: "120px", marginBottom: "15px" }}
            variant="contained"
            color="success"
            onClick={SaveClick}
