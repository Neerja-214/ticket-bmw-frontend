...
const CbwtfMonthlyRptcpcb = () => {
    ...
    const onButtonClicked = (action: string, rw: any) => {
        if (action == "Details") {
            ...
            sessionStorage.setItem('cbwtfAnnlRpt', JSON.stringify(rw));
            navigate("/cbwtfAnnulrptcp");
        }
    };
    ...
    const GetList = () => {
        ...
        let usrnm = getUsrnm();
        let cmpid = getCmpId();
        let payload: any = postLinux(lvl + '=' + who + "=" + year + "=cbwtf_mr=" + usrnm + '=' + cmpid, 'show_AR_filing')
    ...
    const { data: dataSvd1, refetch: refetch } = useQuery({
        queryKey: ["svOldFormHcf", sttValue, cbwtfValue],
        queryFn: GetList,
        enabled: true,
        staleTime: 0,
...
}; export default React.memo(CbwtfMonthlyRptcpcb);