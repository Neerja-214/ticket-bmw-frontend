...
const HcfArDaily = () => {
  ...
  const [showMessage, setShowMessage] = useState<any>({ message: [], type: 'error'})
  ...
  const reqFlds = [
    ...
    { fld: 'whtwt', msg: 'Enter White Weight', chck: 'length' },
  ...
  const coldef = [
    ...
    { field: 'dt_wst', hide: true, width: 180, headerName: 'Date' },
    { field: 'dt_str', hide: false, width: 180, headerName: 'Date' },
    {
      headerName: 'Red',
      children: [
  ...
  const GetData = () => {
    ...
    let dateStr: string = moment(today).format("DD-MMM-yyyy");
    let usrnm = localStorage.getItem('hcfUserName');
    let selectedDate = getFldValue(state.textDts, 'dt_wst') 
    let dt_wst = selectedDate ? selectedDate : dateStr;
    let payload: any = postLinux(usrnm + "=" + dt_wst, 'show_daily_rep');
    return nrjAxiosRequestBio("show_daily_rep", payload);
  };
...
export default React.memo(HcfArDaily);