...
const GridDisplay = (props: any) => {
  ...
  const coldef: any[] = [
    ...
    { field: "stt", hide: false, width: 130, headerName: "State/UT" },
    {
      field: "cty",
      width: 130,
      headerName: "City",
  ...
  const pdfColWidth = ['15%', '10%', '5%', '10%', '10%', '5%', '10%', '10%', '10%', '10%', '10%', '10%']

  const printExcelHeader = ["HCF TYPE", "NO OF BEDS", "HCF", "CITY", "STATE/UT", "REGIONAL DIRECTORATE", "CONTACT PERSON", "ADDRESS I", "ADDRESS II", "ADDRESS III", "PIN CODE", "HCF KEY", "LABEL NO"]
  const keyOrder: string[] = ['hcftyp', 'nobd', 'hcfnm', 'cty', 'stt', 'rgd', 'cntprsn', 'addra', 'addrb', 'addrc', 'pnc', 'hcfkey', 'labelno']
  const excelColWidth = [{ wch: 10 }, { wch: 10 }, { wch: 50 }, { wch: 30 }, { wch: 30 }, { wch: 30 }, { wch: 30 }, { wch: 30 }, { wch: 30 }, { wch: 30 }, { wch: 30 }, { wch: 30 }, { wch: 30 }]
  const onRowSelected = (data: string) => {
    ...
    let tempData = {
      ...
      cntpr: ech[14],
      phn: ech[17],
      hcftyp: ech[5] && ech[13].includes("[") && ech[13].includes("]") ? ech[13].split("[")[1].split("]")[0] : "NA",
      lgt: ech[18],
      ltt: ech[19],
    };
    handleOpen(tempData);
  ...
  const ShowHcfList = (data4: any) => {
    ...
    if (Array.isArray(data4.data) && data4.data.length) {
      ...
      ary = ary.map(item => {
        const fullForm = catFromAbbreviation[item.hcftyp] || ''; // Get full form from catFromAbbreviation
        return {
          ...item, // Spread existing item properties
          tphcf: `${item.tphcf} [${fullForm}]` // Append full form to tphcf
        };
      });
...
<div className="flex justify-center bg-gray-100">
      ...
      {open && (
        ...
        <Modal open={open} size="sm" onClose={handleClose} style={{ marginTop: "40px" }}>
          <Modal.Header>
            <Modal.Title>
      ...
      {open1 && (
         <div className="mt-10 flex justify-center">
        <Modal open={open1} size="md" onClose={handleClose1} style={{ marginTop: "40px" }}>
          <Modal.Header>
            <Modal.Title>
              <div className="font-semibold">
